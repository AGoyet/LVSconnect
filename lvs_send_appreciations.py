...
def get_appr_col(csv_fname):
    ...
    try:
        ...
        raise RuntimeError(
            'Error: CSV file must contain a column named "' + appr_header_string + '"'
        )
...
def send_apprs(
    ...
    if len(overwrite_list) > 0:
        print(
            f"Warning: {len(overwrite_list)} appreciation(s) to upload would OVERWRITE an existing appreciation on website."
        )
    ...
    if len(delete_list) > 0 and not never_delete:
        print(
            f"Warning: {len(delete_list)} appreciation(s) to upload would DELETE an existing appreciation on website."
        )
    ...
    if ask_to_write:
        dialog_s += (
            "Uploading will write "
            + str(write_count)
            + " appreciation(s) to the website.\n"
        )
    if dialog_s != "":
        ...
        if not answer:
            ...
            if delete_count > 0 and ask_to_delete:
                print(
                    "You can upload appreciations without deleting existing ones with the --no-delete option."
                )
            return
    # Actual uploading
    send_apprs_dopost(
        s, trimester, student_names, json_grades, json_apprs, service_id, new_apprs_dict
    )
...
def main():
    try:
        ...
        arg_descs = [
            ...
            (
                ("--write",),
                {
                    ...
                    "help": "Write appreciations to the website. Default is to not delete. --no-write implies --no-delete.",
                },
            ),
            (
                ("--delete", "--overwrite"),
                {
                    "action": argparse.BooleanOptionalAction,
                    "help": "Overwrite existing appreciations on the website (including deleting them if they are not present in the CSV). Default is to ask.",
                },
            ),
        ]
        ...
        args = lvs_get_args(
            ...
            description="Upload student appreciations from a csv file to an axess website.",
            required=["csv_fname"],
        )
