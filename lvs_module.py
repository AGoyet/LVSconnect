...
def get_config_dict_from_files():
    ...
    config_locations = [
        ...
        ),  # dir in which program file is located
...
shared_arg_descs = [
    (
        ("-t", "--trimester"),
        {
            "type": int,
            "choices": [1, 2, 3],
            "help": "Trimester. Must be 1, 2 or 3. Default is to guess from the csv file name or current date.",
        },
    ),
    (
        ("-g", "--group"),
    ...
    (
        ("-u", "--user"),
        {
            "help": "The user for login. Without this option, the program will prompt for login info."
        },
    ),
    (
        ("-p", "--password"),
        {
            "help": "The password for login. Without this option, the program will prompt for login info."
        },
    ),
    (
        ("--login_url",),
...
def lvs_get_args(
    ...
    args2 = {
        k: v for (k, v) in args.items() if (not k in config_dict) or (not v is None)
    }
    args = {}
    ...
    if should_process("csv_fname") or should_process("csv_file"):
        if args["csv_fname"] is None:
            args["csv_fname"] = get_csv_filename(
                prompt_if_notfound=prompt_csv, silent=silent_csv, confirm=confirm_csv
            )
    ...
    if should_process("trimester"):
        ...
        if args["trimester"] is None:
            args["trimester"] = guess_trimester_from_date()
    return args
...
# get necessary arguments from the command line "args" dict directly
def open_session_from_args(args):
    needed_args = [
        "user",
        "password",
        "login_url",
...
def get_csv_filename(prompt_if_notfound=True, silent=False, confirm=False):
    ...
    if not prompt_if_notfound:
        ...
        if multiple_csv_flag:
            if not silent:
                print(
                    "Warning: Multiple CSV files found. Proceeding without picking one"
                )
            return ""
    got_from_dialog_flag = False
    if csv_fname == "":
        print(
            f"No csv file specified as an argument, and none found in working directory {os.getcwd()}."
        )
        csv_fname = input_open_file(
        ...
        print(
            f"No csv file specified as an argument, and multiple files found in working directory {os.getcwd()}."
        )
        answer = input_open_file(
            f'Enter the name of the CSV file. Default is to use "{csv_fname}":\n',
            filetypes=[("CSV files", ".csv"), ("all files", "*")],
        )
        if answer != "":
            csv_fname = answer
...
def get_trimester_from_json(json_groups):
    ...
    for periode in group["periodes"]:
        ...
        if periode["isParDefaut"] == True:
            default_t = t
        if (
            len(periode["verrouillages"]) != 0
            and periode["verrouillages"][0]["verrouille"] == False
        ):
            all_locked = False
    if default_t == 0:
        raise RuntimeError(
            "Unexpected format on website. Unable to get trimesters information."
        )
    if all_locked:
        print(
            f"No trimester provided. Guessing trimester 3 from website (all trimesters are locked, so assuming year has ended)."
        )
        return 3
...
def get_trimester_nb_from_fname(fname):
    m = re.search(trimester_regex(), fname)
    if not m:
        return None
    trimester_s = m.group()
...
def get_trimester_from_csv_fname(csv_fname):
    trimester = get_trimester_nb_from_fname(csv_fname)
    if trimester is None:
        raise RuntimeError(
            f'Unable to guess trimester from csv file name "{csv_fname}". The trimester must be explicitely given. Launch the program with -h to see usage.'
        )
...
def get_mess_dest_json(s, dest_search_s, dest_type="student"):
    ...
    if not dest_type in dest_types:
        raise Exception(
            f"Incorrect destinatory type (should be in {list(dest_type.keys())})"
        )
    ...
    if len(json_dest_search_res) > 1:
        raise Exception(
            'Multiple results for destinatory search "' + dest_search_s + '"'
        )
...
@pronote.reimplemented
def send_message(
    s, dest_search_s, message_subject, message_body, dest_type="student", enseigne=None
):
    ...
    r = s.post(
        get_url("dest_add").format(json_new_message["id"]), json=json_payload_dest_add
    )
    r.raise_for_status()
    ...
    r = s.post(
        get_url("compose").format(json_new_message["id"]), json=json_payload_compose
    )
    r.raise_for_status()
...
def student_rows_of_csv_rows(rows):
    ...
    for row in rows[1:]:
        ...
        if (
            csv_name == "Moyenne"
            or csv_name == "Note min | Note max"
            or csv_name == "Moy. du groupe :"
        ):
            break
...
def match_group_to_classgroup(classgroups, group_name):
    ...
    raise RuntimeError(
        f'Unable to match group name "{group_name}" to one of the possible class group names (full list: {classgroup_names}). You can specify which class group to use with the -g option, or by modifying the first cell of the CSV.'
    )
