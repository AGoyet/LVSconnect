...
def add_tt(tt1, tt2):
    h1, m1 = tt1
    h2, m2 = tt2
    return (h1 + h2 + (m1 + m2) // 60, (m1 + m2) % 60)
...
def guess_base_time_slots(time_slots):
    ...
    while si < len(start_times):
        ...
        if ei >= len(end_times):
            print(
                f"Unexpected times in schedules, ignoring start time {s_of_tt(start_times[si])}"
            )
            break
...
def find_free_rooms(
    time_slots, rooms, date_tuple, start_tt, excluded=set(), duration=21, max_delay=30
):
    ...
    for tt in possible_starts:
        free_rooms_by_start[tt] = find_free_rooms_sub(
            time_slots, rooms, date_tuple, tt, duration=duration
        )
    ...
    for tt in possible_starts:
        pruned[tt] = (
            free_rooms_by_start[tt] - free_rooms_by_start.get(prev_tt, set()) - excluded
        )
        # Remove empty times
        if not pruned[tt]:
            del pruned[tt]
...
def load_time_slots(fname, time_slots, always_free, update_times, rooms):
    ...
    try:
        ...
        raise RuntimeError(
            f"Error load data from file {fname}. This file must be empty or contain data created by this program."
        )
    time_slots.update(a)
...
def find_and_display_free_rooms(
    ...
    if load_file:
        ...
        new_time_slots, new_always_free = get_all_time_slots(
            s, rooms, excluded=excluded
        )
    ...
    free_rooms_by_start = find_free_rooms(
        time_slots,
        rooms,
        date_tuple,
        start_tt,
    ...
    for d in time_slots:
        ...
        for room_name in rooms:
            schedule = [
                slot for slot in time_slots[d] if room_name in time_slots[d][slot]
            ]
            schedule.sort()
    ...
    for d in time_slots:
        for slot in time_slots[d]:
            all_slots.add(slot)
    whole_day = (
        min([slot[0] for slot in all_slots]),
        max([slot[1] for slot in all_slots]),
    )
    # Display result
    result_s = ""
    result_s += s_of_free_rooms(
        free_rooms_by_start,
        date_tuple,
        room_schedules[date_tuple],
        whole_day,
        are_all_free,
        always_free=always_free,
    )
    ...
    try:
        ...
        arg_descs = [
            ...
            (
                ...
                {
                    "action": "store_true",
                    "help": "Use current date and time, without any dialog.",
                },
            ),
            ...
            (
                ...
                {"metavar": "DATE", "help": "Format is DD/MM/YYYY."},
            ),
            (
                ("--excluded-rooms",),
                {
            ...
            (
                ("--duration",),
                {
                    "type": int,
                    "default": 21,
                    "help": "How long the room is needed in minutes. Low values might lead to include recess time. Default is 21.",
                },
            ),
            (
                ("--max-delay",),
                {
                    "type": int,
                    "default": 30,
                    "help": "The program displays additional rooms after those that are free now. This sets the maximum amount of time (in minutes) to look for those rooms. Default is 30.",
                },
            ),
            (
                ("--output",),
                {
                    "metavar": "FILE",
                    "type": str,
                    "help": "Write the final result at the end of the given file.",
                },
            ),
            (
                ("--load",),
                {
                    "metavar": "FILE",
                    "type": str,
                    "help": "Load data from the given file instead of downloading from the website.",
                },
            ),
            (
                ("--save",),
                {
                    "metavar": "FILE",
                    "type": str,
                    "help": "Save the data downloaded from the website to the given file then exit. If the file already exists, it will be updated.",
                },
            ),
        ]
        shared_args = []
        ...
        args = lvs_get_args(
            ...
            description="Downloads the time schedule for each room then displays the ones which are free at a given date and time. Without any arguments, asks for date and time.",
        )
        start_tt = None
        date_tuple = None
        curr_d = datetime.datetime.now()
        if args["now"]:
            ...
            if not choice:
                # Ask for date
                args["date"] = input_date_dmy(
                    prompt="Enter the date for which to check for free rooms."
                )
                # Ask for time
        ...
        if args["date"]:
            ...
            if not date_tuple:
                raise RuntimeError(
                    f"Invalid format for date string (should be DD/MM/YYYY): {args['date']}"
                )
            if date_tuple[2] < 100:
        ...
        if args["time"]:
            ...
            if not start_tt:
                raise RuntimeError(
                    f"Invalid format for time string (should be HH:MM): {args['time']}"
                )
        if not start_tt:
        ...
        result_s = find_and_display_free_rooms(
            s,
            date_tuple,
            start_tt,
            excluded=excluded,
