...
def get_evaluations_from_csv(csv_fname):
    ...
    if len(rows) < 2:
        raise RuntimeError("Unexpected csv file format (no second line with evaluation descriptions)")
    evaluation_descs={}
    for i in range(1, len(rows[1])):
        ...
        if evaluation_name in evaluation_descs.keys():
            raise RuntimeError(f'Evaluation name "{evaluation_name}" appears multiple times in first row of csv file.')
        evaluation_descs[evaluation_name]= desc
    print("Found",len(evaluation_descs),"evaluations in the csv file: ", ", ".join(evaluation_descs.keys()))
...
@pronote.notimplemented
def modify_evaluation_desc(s, json_grades, service_id, trimester, evaluation_name, max_grade, coefficient):
    devoir= None
...
def get_evaluation_website_descs(json_grades):
    website_descs= { devoir["titre"]:(devoir["noteMaximalEvaluation"], devoir["coefficient"], devoir["id"])
                     for devoir in json_grades["evaluations"]}
...
def get_evaluation_id_and_create_evaluations(s, service_id, trimester, json_grades, evaluation_descs,
                                             create_evaluations= False, hidden= False):
    created_flag= False
    ...
    if evaluations_not_in_csv:
        print(f"WARNING: {len(evaluations_not_in_csv)} evaluation(s) are present on the website but not in the csv file: {', '.join(evaluations_not_in_csv)}")
    if evaluations_not_in_website:
        print(f"Found {len(evaluations_not_in_website)} evaluation(s) not present on the website: {', '.join(evaluations_not_in_website)}")
        if create_evaluations:
            ...
            for evaluation_name in evaluations_not_in_website:
                ...
                desc_full= create_evaluation(s, service_id, trimester, evaluation_name, desc, hidden=hidden)
                evaluation_descs_full[evaluation_name] = desc_full
                created_flag= True
    if created_flag:
        print("Evaluation(s) successfully created. Note that their creation date has been set to today.")
    if evaluations_with_new_desc:
        print(f"Found {len(evaluations_with_new_desc)} evaluation(s) with max grade or coefficient different than on the website: {', '.join(evaluations_with_new_desc)}")
        if create_evaluation:
            ...
            if answer:
                ...
                for evaluation_name in evaluations_with_new_desc:
                    ...
                    modify_evaluation_desc(s, json_grades, service_id, trimester, evaluation_name, max_grade, coefficient)
...
def send_grades(s, csv_fname, trimester, group_name,
    ...
    created_flag, evaluation_descs_full= get_evaluation_id_and_create_evaluations(s, service_id, trimester, json_grades, evaluation_descs, create_evaluations=create_evaluations, hidden=hidden)
    ...
    new_grades_dict= {}
    for evaluation_name, (evaluation_col, max_grade, coefficient, evaluation_id) in evaluation_descs_full.items():
        write_list= []        
        overwrite_list= []
        delete_list= []
        for (student_id, row) in row_of_student_id.items():
            ...
            try:
                ...
                if g > max_grade:
                    print(f"Warning: In evaluation \"{evaluation_name}\", grade \"{grade_csv}\" in csv file is greater than the maximum grade of {max_grade}. Replacing it with {max_grade}")
...
#                        'help':'If provided, only this evaluation (evaluation) will be modified on the website.'}),            
        ...
        send_grades(s, args["csv_fname"], args["trimester"], args["group_name"],
                    create_evaluations= args["create"], hidden= args["hidden"],
