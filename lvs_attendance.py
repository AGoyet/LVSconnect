...
add_url(
    "attendance_choixClasseEleveStrater", "/vsn.main/absence/choixClasseEleveStrater"
)
...
def get_lsl_appr_cols_and_discs(csv_fname):
    ...
    if len(rows) < 2:
        raise RuntimeError(
            "Unexpected csv file format (no second line with test descriptions)"
        )
    indices = all_indices(rows[1], lsl_sub_header)
    if not indices:
        raise RuntimeError(
            f'Error: CSV file must contain columns with the name of the discipline in the first cell and the text"{lsl_sub_header}" in the second cell.'
        )
...
def mandatory_get_attendance_classgroups(s):
    ...
    r1 = s.post(
        "https://lyceepaullapie92.la-vie-scolaire.fr/vsn.main/WSMenu/getModuleUrl?mod=ABSENCES&minuteEcartGMTClient=-120"
    )
    r1.raise_for_status()
    r2 = s.get(json.loads(r1.text)["location"])  # redirects
    r2.raise_for_status()
    r3 = s.get(
        "https://lyceepaullapie92.la-vie-scolaire.fr/vsn.main/absence/absenceStart?actionEnd=calendrierAbsenceEleve&type=absence&idEleve=&accesDeMenu=true"
    )  # also mandatory
    r3.raise_for_status()
    ...
    d = {
        ...
        for option in e.find_all("option")
...
def get_all_students_ids(s, classgroup_id):
    ...
    if len(selects) != 1:
        raise RuntimeError(
            "Unexpected format for attendance initial display on website"
        )
...
def get_attendance_for_last_month(attendance_dict, s, classgroup_id):
    ...
    for tr in trs:
        ...
        for span in spans:
            ...
            if m is None:
                raise RuntimeError(
                    "Unexpected format of student calendar view (couldn't find date in cell)"
                )
            date = m.group()
...
def parse_student_calendar(attendance_dict, soup, student_name):
    ...
    if len(tables) != 1:
        raise RuntimeError(
            "Unexpected format of student calendar view (no table tag found)"
        )
...
def get_student_names_to_check_from_csv(csv_fname, test_name=None):
    ...
    if test_name is None:
        ...
        if not test_names:
            raise RuntimeError("CSV file does not contain any evaluations (tests).")
        test_name = input_pick_option(
            test_names, prompt="Evaluation (test) name not provided. Choose one:"
        )
    test_col = -1
    for i in range(1, len(rows[0])):
        if rows[0][i] == test_name:
            if test_col != -1:
                raise RuntimeError(
                    f"Evaluation {test_name} appears more than once in CSV file."
                )
            test_col = i
    if test_col == -1:
...
def get_student_names_to_check_from_json(s, json_grades, test_id):
    ...
    for student_id, student_name in student_names_of_ids.items():
        ...
        if not key in grades_website:
            raise RuntimeError(
                f"Unexpected website response (valid student id has no grade for a valid test id ({key}))"
            )
...
def collect_all_necessary_params(
    s,
    classgroups,
    group_name=None,
    test_name=None,
    csv_fname=None,
    do_all_students=False,
    trimester=None,
    test_date=None,
):
    ...
    if group_name is None:
        if csv_fname is not None:
            ...
            group_name = input_pick_option(
                service_id_of_group_name.keys(),
    ...
    if (not test_name) and (not do_all_students):
        ...
        choice = input_Yn(
            'Select evaluation form a list? (Choosing "No" will get attendance from all students from a group.)'
        )
        do_all_students = not choice
    if do_all_students:
        if not test_date:
            print("No date provided.")
            test_date = input_date_dmy(
                prompt="Please input the date for which to check attendance."
            )
            if not test_date:
                raise RuntimeError("No date provided. Aborting.")
        service_id = service_id_of_group_name[group_name]
        json_grades = get_grades(
            s, service_id, 1
        )  # It should be ok to get grades from trimester=1 every time.
        ...
        if (not student_names_to_check) or (not grades_dict) or (not test_date):
            if trimester is None:
                ...
                choice = input_Yn(
                    f"No trimester provided. Guessed trimester {trimester} based on current date. Is this correct?"
                )
                if not choice:
                    choice2 = input_pick_option(
                        ["Trimester 1", "Trimester 2", "Trimester 3"],
                        prompt="Choose trimester:",
                    )
                    trimester = int(choice2[-1:])
            # We have service_id
            json_grades = get_grades(s, service_id, trimester)
            test_id_of_name = {
                devoir["titre"]: devoir["id"] for devoir in json_grades["evaluations"]
            }
            if not test_name:
                ...
                test_name = input_pick_option(
                    options, prompt="Evaluation (test) name not provided. Choose one:"
                )
            if not test_name in test_id_of_name:
                raise RuntimeError(
                    f"Provided evaluation (test) name {test_name} not found on website. If the name is correct, either create it on the website or provide a CSV file that includes this evaluation name."
                )
            ...
            if (not student_names_to_check) or (not grades_dict):
                (
                    student_names_to_check,
                    grades_dict,
                ) = get_student_names_to_check_from_json(s, json_grades, test_id)
            if not test_date:
                date_ymd = get_date_from_json_ymd(json_grades, test_id)
                test_date = convert_date_from_ymd(date_ymd)
                choice = input_Yn(
                    f"Evaluation (test) date not provided. Guessed date {test_date} from website. Is this correct?"
                )
                if not choice:
                    test_date = input_date_dmy(
                        prompt="Please input the date for which to check attendance."
...
def get_attendances(s, classgroups, student_names_to_check):
    ...
    if students_not_found:
        print(
            f"Warning: The following students were not on the attendance lists: {nicer_str(students_not_found)}"
        )
    return (attendance_dict, students_not_found)


def output_attendance_sub(
    attendance_dict,
    test_date,
    group_name,
    test_name,
...
def output_attendance(
    ...
    output_s = output_attendance_sub(
        attendance_dict,
        test_date,
        group_name,
        test_name,
    ...
    if output_file is None or output_file == "":
        print(
            "\n" + output_s
        )  # Leave empty line before and after in stdout for readability.
    if output_file is None:
        choice = input_yN(prompt="The output is being displayed. Save it to a file?")
        if choice:
            output_file = input_save_file(
                "Chose a file to save the output:",
...
def main():
    try:
        ...
        args = lvs_get_args(
            ...
            description="Checks for student's attendance on an axess website. Will interactively prompt for most arguments if not given.",
            prompt_csv=False,
            silent_csv=True,
            confirm_csv=True,
        )
        ...
        params = collect_all_necessary_params(
            s,
            classgroups,
        ...
        attendance_dict, students_not_found = get_attendances(
            s, classgroups, student_names_to_check
        )
        output_attendance(
            attendance_dict,
            test_date,
