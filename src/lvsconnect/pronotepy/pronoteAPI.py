...
class _Communication(object):
    ...
    def post(
        ...
        if "Erreur" in r_json_raw:
            r_json = r_json_raw
            log.debug(f"[_Communication.post] PRONOTE API Error received: {r_json}")
        ...
        if self.compress_requests:
            ...
            try:
                ...
                log.debug(f"[_Communication.post] JSONDecodeError on payload: {d!r}")
                raise PronoteAPIError("JSONDecodeError while requesting from pronote.")

        log.debug(f"[_Communication.post] Decrypted response: {response_data}")
    ...
    def alive(self) -> None:
        while self.keep_alive:
            ...
            if time() - self._client.communication.last_ping >= 110:
                self._client.post("Navigation", 7, {"onglet": 7, "ongletPrec": 7})
            sleep(1)
